...
//  y su condición de IVA (necesaria para calcular la letra de
//  factura correcta al facturar por WhatsApp).
...
router.get('/api/whatsapp/estado', autenticar, async (req, res) => {
  try {
    const cliente = await db.prepare(
      'SELECT whatsapp_telefono, condicion_iva_negocio FROM clientes WHERE id = ?'
    ).get(req.clienteId);
    ...
    res.json({
      ...
      condicionIva: cliente?.condicion_iva_negocio || null
    });
  } catch (err) {
...
// ══════════════════════════════════════════════════════════
//  POST /api/whatsapp/condicion-iva   (requiere ser el dueño)
//  Guarda la condición de IVA del NEGOCIO (no del cliente al que
//  se le vende). La necesita el webhook de WhatsApp para calcular
//  la letra de factura correcta (Monotributo → C, el resto → B a
//  consumidor final), porque a diferencia de la web, el webhook
//  no tiene acceso al localStorage del navegador.
// ══════════════════════════════════════════════════════════
router.post('/api/whatsapp/condicion-iva', autenticar, soloDueno, async (req, res) => {
  try {
    const { condicionIva } = req.body;
    if (!['Monotributo', 'Responsable Inscripto'].includes(condicionIva)) {
      return res.status(400).json({ error: 'Condición de IVA inválida.' });
    }
    await db.prepare(
      'UPDATE clientes SET condicion_iva_negocio = ? WHERE id = ?'
    ).run(condicionIva, req.clienteId);
    res.json({ ok: true, condicionIva });
  } catch (err) {
    console.error('Error en /api/whatsapp/condicion-iva:', err);
    res.status(500).json({ error: 'No se pudo guardar la condición de IVA.' });
  }
});
