...
// ══════════════════════════════════════════════════════════
app.post('/api/afip-generar-csr', autenticar, (req, res) => {
  try {
    ...
    // 1) Generar la clave privada (2048 bits, formato estándar para AFIP)
    const keyPem = execSync('openssl genrsa 2048', { encoding: 'utf8' });

    // 2) Generar el CSR usando esa clave (todo en memoria, sin guardar archivos en disco)
    const csrPem = execSync(
      `openssl req -new -key /dev/stdin -subj "/CN=Rubrex${orgLimpia}/O=${orgLimpia}/C=AR"`,
      { input: keyPem, encoding: 'utf8' }
    );

    // 3) Guardamos la clave ENCRIPTADA en la base de datos.
...
app.post('/api/afip-credenciales', autenticar, (req, res) => {
  try {
    ...
    if (afip_key) {
      ...
      const actual = db.prepare('SELECT afip_key FROM clientes WHERE id = ?').get(req.clienteId);
