...
app.get('/api/perfil', autenticar, async (req, res) => {
  try {
    const cliente = await db.prepare('SELECT nombre, nombre_display, avatar FROM clientes WHERE id = ?').get(req.clienteId);
    if (!cliente) return res.status(404).json({ error: 'Cliente no encontrado.' });
...
//  Body: { nombreDisplay, nombreNegocio, avatar }
//  - nombreDisplay / nombreNegocio: si no vienen en el body, no se tocan.
//  - avatar: string base64 (data URL) para actualizarla, o null para
//    quitar la foto. Si la clave "avatar" no viene en el body, no se toca.
//
...
//  la fila del dueño — hoy el perfil (nombre/foto) es uno solo por negocio,
//  no por empleado. Si más adelante se quiere un perfil por persona del
//  equipo, hay que sumar estas columnas a la tabla "equipo" también.
// ══════════════════════════════════════════════════════════
app.post('/api/perfil', autenticar, async (req, res) => {
  try {
    const { nombreDisplay, nombreNegocio, avatar } = req.body;
