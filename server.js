...
const app = express();
// Render (como la mayoría de los hosts) pone un proxy/load balancer
// adelante de la app. Sin esto, req.protocol siempre da "http" aunque
// el usuario entre por "https", lo que rompe la validación de firma
// de Twilio en whatsapp-webhook.js (la URL calculada no coincide con
// la URL real que Twilio usó para firmar el request).
...
app.use(require('./factura-routes')); // 👈 nuevo: extracción de facturas de compra con IA (PDF/imagen)
app.use(require('./pagos-routes')); // suscripciones con MercadoPago
app.use(require('./whatsapp-routes')); // 👈 nuevo: vinculación de cuenta con WhatsApp (paso 1 de facturar por WhatsApp)
app.use(require('./whatsapp-webhook')); // 👈 nuevo: recibe los mensajes de WhatsApp (Twilio)
...
app.post('/api/facturar', autenticar, async (req, res) => {
  ...
  try {
    const { total, tipoFactura } = req.body;
    const resultado = await emitirFactura(req.clienteId, { total, tipoFactura });
    res.json({ ok: true, ...resultado });
  } catch (err) {
    if (err instanceof ErrorFacturacion) {
