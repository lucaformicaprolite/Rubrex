...
async function init() {
  ...
  // ── Equipo (empleados que puede sumar el dueño) ──
  // Esta tabla nunca se había creado acá (por eso fallaba /api/equipo).
  // Columnas según lo que usa equipo-routes.js.
  await pool.query(`
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_equipo_cliente ON equipo(cliente_id)`);

  // Asegura que la restricción de "rol" permita 'general', sin importar si
  // la tabla ya existía antes con una restricción vieja o se acaba de crear
  // recién arriba. Se saca cualquier restricción CHECK previa sobre "rol"
  // y se agrega siempre la misma, actualizada.
  await pool.query(`
    DO $$
    DECLARE
      cname text;
    BEGIN
      FOR cname IN
        ...
        WHERE conrelid = 'equipo'::regclass AND contype = 'c'
      LOOP
        EXECUTE format('ALTER TABLE equipo DROP CONSTRAINT %I', cname);
      END LOOP;

      ALTER TABLE equipo ADD CONSTRAINT equipo_rol_check
        CHECK (rol IN ('dueno','cajero','deposito','general'));
    END $$;
  `);
}
