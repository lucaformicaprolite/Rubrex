...
async function init() {
  ...
  // Migración: datos del comprador y N° de orden cuando la venta llega
  // desde una tienda online (Tienda Nube / Mercado Libre / WooCommerce).
  // No existían antes porque el webhook los recibía y los descartaba.
  await pool.query(`ALTER TABLE ventas ADD COLUMN IF NOT EXISTS external_order_id TEXT`);
  await pool.query(`ALTER TABLE ventas ADD COLUMN IF NOT EXISTS comprador JSONB`);
