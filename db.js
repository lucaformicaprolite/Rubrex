...
async function init() {
  ...
  // ── Arreglo: la tabla "equipo" quedó con una restricción vieja en la
  // columna "rol" que no incluía 'general' (se agregó después en el código
  // pero nunca se actualizó la restricción en la base). Esto la corrige
  // sola, sin tocar nada a mano en la base de datos.
  await pool.query(`
    DO $$
    DECLARE
      cname text;
