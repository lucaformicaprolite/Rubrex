...
async function init() {
  ...
  // whatsapp_telefono: el número REAL vinculado a la cuenta (formato E.164,
  // ej "5493544644461"), distinto de "telefono" de arriba (que es solo un
  // dato de perfil, no verificado). Es lo que usamos para saber, cuando
  // llega un mensaje al webhook, a qué negocio pertenece.
  await pool.query(`ALTER TABLE clientes ADD COLUMN IF NOT EXISTS whatsapp_telefono TEXT`);
  await pool.query(`
    CREATE UNIQUE INDEX IF NOT EXISTS idx_clientes_whatsapp_telefono
    ON clientes(whatsapp_telefono) WHERE whatsapp_telefono IS NOT NULL
  `);
  // Código corto que el dueño genera desde la app (ej "RBX-7X2K") y le manda
  // por WhatsApp al bot para vincular su número, sin tener que mandar su
  // contraseña por chat. Con expiración corta (ver whatsapp_codigo_expira).
  await pool.query(`ALTER TABLE clientes ADD COLUMN IF NOT EXISTS whatsapp_codigo_vinculacion TEXT`);
  await pool.query(`ALTER TABLE clientes ADD COLUMN IF NOT EXISTS whatsapp_codigo_expira TIMESTAMP`);
  // Condición de IVA del NEGOCIO (no del cliente al que se le vende).
  // Hasta ahora esto solo vivía en localStorage del navegador (ver
  // getCondicionNegocio() en el frontend), lo cual funciona para
  // facturar desde la web pero es inútil para el webhook de WhatsApp,
  // que corre en el servidor. Valores esperados: 'Monotributo' o
  // 'Responsable Inscripto' (mismos strings que ya usa el frontend).
  await pool.query(`ALTER TABLE clientes ADD COLUMN IF NOT EXISTS condicion_iva_negocio TEXT`);
  ...
  // Para la campanita de notificaciones (stock bajo). Default 5 para que
  // los productos ya cargados antes de este campo también disparen aviso.
  await pool.query(`ALTER TABLE productos ADD COLUMN IF NOT EXISTS stock_minimo INTEGER NOT NULL DEFAULT 5`);
  ...
  // ── Borradores de factura por WhatsApp ──
  // El webhook de WhatsApp es "sin estado": cada mensaje llega como un
  // request nuevo, sin sesión de navegador. Cuando la IA interpreta un
  // pedido ("facturame $5000 por un corte de pelo") y arma los datos,
  // los guardamos ACÁ mientras esperamos que el usuario confirme con el
  // siguiente mensaje ("sí"). Con expiración corta: si no confirma en
  // unos minutos, el borrador se descarta y hay que empezar de nuevo.
  await pool.query(`
    CREATE TABLE IF NOT EXISTS whatsapp_borradores (
      id SERIAL PRIMARY KEY,
      negocio_id INTEGER NOT NULL REFERENCES clientes(id) ON DELETE CASCADE,
      telefono TEXT NOT NULL,
      datos JSONB NOT NULL,       -- { total, descripcion, tipoFactura, ... }
      estado TEXT DEFAULT 'pendiente_confirmacion',
      expira TIMESTAMP NOT NULL,
      creado_en TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    )
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_wa_borradores_telefono ON whatsapp_borradores(telefono)`);
