...
router.post('/api/copiloto/confirmar', autenticar, async (req, res) => {
  ...
  try {
    ...
    if (tipo === 'dar_baja_producto') {
      if (!['dueno', 'deposito', 'general'].includes(rol)) return res.status(403).json({ ok: false, error: 'Tu rol no puede eliminar productos.' });
    ...
    if (tipo === 'registrar_compra') {
      if (!['dueno', 'deposito', 'general'].includes(rol)) return res.status(403).json({ ok: false, error: 'Tu rol no puede registrar compras.' });
    ...
    if (tipo === 'crear_producto') {
      if (!['dueno', 'deposito', 'general'].includes(rol)) return res.status(403).json({ ok: false, error: 'Tu rol no puede crear productos.' });
