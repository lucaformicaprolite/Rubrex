...
const { emitirFactura, ErrorFacturacion, calcularTipoFacturaPorDefecto } = require('./facturacion');
  ...
  async (req, res) => {
    try {
      ...
      const cliente = await db.prepare(
        'SELECT id, nombre, condicion_iva_negocio FROM clientes WHERE whatsapp_telefono = ?'
      ).get(telefono);
      ...
      if (!cliente.condicion_iva_negocio) {
        return responderTwiML(
          res,
          'Antes de facturar por acá necesito saber tu condición de IVA (Monotributo o Responsable Inscripto), ' +
          'para saber qué letra de factura te corresponde. Configurala en Rubrex → Mi perfil → WhatsApp.'
        );
      }
      ...
      const tipoFacturaFinal = interpretado.tipoFactura || calcularTipoFacturaPorDefecto(cliente.condicion_iva_negocio);
      await db.prepare(`
      ...
      return responderTwiML(
        res,
        `Vas a facturar $${formatearPesos(interpretado.total)} (Factura ${tipoFacturaFinal}) ` +
